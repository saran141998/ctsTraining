...
namespace linq
{
	...
	class Program
	{
		static void Main(string[] args)
		{
			...
			d.ToShortDateString();
			Predicate<course> fst = x => x.Id == 102;
			var st = c.Find(fst);
			Console.WriteLine("{0}\t{1}", st.CourseName, st.fee);
			//int[] a = { 5, 4, 1, 7, 6, 3, 2 };
			...
			{
				Console.WriteLine("{0}\t{1}\t{2}\t{3}\t{4}", q.Id, q.CourseName, q.fee, q.startDate.ToShortDateString(), q.endDate.ToShortDateString());
			}
			Console.ReadKey();
		}
	}
